...
const DRAFT_STORAGE_KEY = "linda_vision_section_draft";
...
export default function AdminVisionPage() {
      ...
      if (savedDraft) {
        try {
          ...
          if (parsed && typeof parsed === "object") {
            setFormData({
              vision_image_url: parsed.vision_image_url || aboutData?.vision_image_url || "",
              vision_heading: parsed.vision_heading || aboutData?.vision_heading || "",
              vision_paragraph: parsed.vision_paragraph || aboutData?.vision_paragraph || "",
            });
            setHasDraft(true);
  ...
  const handleChange = (field: "vision_image_url" | "vision_heading" | "vision_paragraph", value: string) => {
    setFormData((prev) => {
      const updated = {
        ...prev,
        [field]: value,
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (formData.vision_image_url) {
        await deleteFromCloudinary(formData.vision_image_url);
      }

      handleChange("vision_image_url", json.url);
      setStatusMessage({ type: "success", text: "Vision image uploaded successfully!" });
    } catch (err: unknown) {
  ...
  const handleRemoveImage = async () => {
    if (!formData.vision_image_url) return;
    const oldUrl = formData.vision_image_url;
    handleChange("vision_image_url", "");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        vision_heading: formData.vision_heading.trim(),
        vision_paragraph: formData.vision_paragraph.trim(),
        vision_image_url: formData.vision_image_url,
      };

      const saved = await saveAbout(payload);
      if (saved) {
        setFormData({
          vision_image_url: saved.vision_image_url || "",
          vision_heading: saved.vision_heading || "",
          vision_paragraph: saved.vision_paragraph || "",
        });
      }
      if (typeof window !== "undefined") {
        localStorage.removeItem(DRAFT_STORAGE_KEY);
      ...
      setStatusMessage({
        type: "success",
        text: "Vision section saved successfully!",
      });
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : "Failed to save vision section";
      setStatusMessage({ type: "error", text: msg });
    } finally {
      setSaving(false);
    }
    ...
    return (
      <div className="min-h-screen bg-neutral-50 flex items-center justify-center p-4">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm font-medium text-neutral-500">Loading Vision Section...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-neutral-100 flex flex-col md:flex-row text-neutral-900 md:h-screen md:overflow-hidden">
      ...
      <main className="flex-1 flex flex-col min-w-0 md:h-screen overflow-hidden">
        {/* Header */}
        <AdminHeader title="Vision Section Management" />
          ...
          <div className="max-w-5xl w-full mx-auto space-y-6">
            ...
            {hasDraft && (
              <div className="p-3.5 sm:p-4 rounded-sm bg-amber-50/90 border border-amber-300 text-amber-900 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs shadow-xs animate-fade-in">
                <div className="flex items-center gap-2.5">
                  ...
                  <span>
                    <strong>Unsaved draft restored:</strong> Your typed changes are saved locally. Click <strong>Save Vision Section</strong> to update the database.
                  </span>
                </div>
                <button
                  type="button"
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="bg-amber-50/70 border border-amber-200 rounded-sm p-4 text-xs text-amber-900 flex items-center gap-3">
                <Compass className="w-5 h-5 text-[#FF9E15] shrink-0" />
                <p>
                  <strong>Frontend Layout:</strong> The Vision section will display with the <strong>Image on the Left</strong> and the <strong>Content (Heading & Description) on the Right</strong>.
                </p>
              </div>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => handleChange("vision_heading", e.target.value)}
                      placeholder="e.g. Our Vision for Living Spaces"
                  ...
                  <div>
                    ...
                    <textarea
                      rows={6}
                      value={formData.vision_paragraph}
                      onChange={(e) => handleChange("vision_paragraph", e.target.value)}
                      placeholder="Detail your company's vision, philosophy, aspirations, and future outlook..."
